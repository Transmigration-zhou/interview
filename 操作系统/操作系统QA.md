...
select函数作用是将 fd 的集合（bitmap）直接从用户态拷贝到内核态，由内核去遍历判断哪一个 fd 有数据。
...
epoll可以理解为event pool，不同与select、poll的轮询机制，epoll采用的是事件驱动机制，每个fd上有回调函数，当接收到数据时会回调该函数，同时将该fd的引用放入rdlist就绪列表中，时间复杂度为O(1)。
