...
https://time.geekbang.org/column/article/319988

事务的隔离级别主要为了解决三类问题（并行事务的情况下）：

- 脏读：一个事务读到另外一个事务还==没有提交==的数据。

  ![img](https://pic3.zhimg.com/v2-0615e8417fec4174fab1a1b6519ec8da_r.jpg)

- 不可重复读：一个事务先后执行同一条 SQL，但两次读取到的数据不同。 （针对更新操作）

  ![img](https://pic3.zhimg.com/v2-ff1bf23ff6d0db40133204ef885a3b02_r.jpg)

- 幻读：前后读取的记录数量不一致。（针对插入和删除操作）

  - 比如在一个事务中明明没有查到主键为 X 的数据，但主键为 X 的数据就是插入不进去，就像某种幻觉一样。
...
因为四个隔离级别是通过加锁解决的并发问题，MySQL 中的锁可以分为共享锁/读锁、排它锁/写锁、间隙锁/范围锁、行锁和表锁。

- 读未提交写的时候只加写锁，而且一直持续到事务结束，但==完全不加读锁==。
- 读已提交写的时候加写锁，而且一直持续到事务结束。读的时候加读锁，读锁在查询结束马上释放。
- 可重复读对事务所涉及到的数据加读锁和写锁，并且一直持续到事务结束，但不再加范围锁。
- 可串行化对事务所有读、写的数据全都加上读锁、写锁和范围锁。

（其中读已提交、可重复读会使用的是 MVCC 进行实现的。）

#### 共享锁（读锁、S锁）、排它锁（写锁、X锁）、间隙锁（范围锁）

- 读锁是针对同一份数据，多个读操作可以同时进行，简单来说即读加锁，==不能写并且可并行读==。

- 写锁针对写操作，假如当前写操作没有完成，那么它会阻断其它的写锁和读锁，即写加锁，其它==读写都阻塞==。

- 间隙锁：对于某个范围直接加排他锁，在这个范围内的数据不能被读取，也不能被写入。

  间隙锁分为两种：`Gap Locks`和`Next-Key Locks`。
...
- MVCC（Multi-Version Concurrency Control ）就是多版本并发控制。MVCC 通过在数据库中保存数据的多个版本（多版本依赖 undo log 来实现）来实现并发控制，每个事务在读取数据时，看到的是数据的一个快照，而不是实时的数据，这样可以避免幻读问题的出现。

- MVCC只有在InnoDB的可重复读（RR）和读已提交（RC）隔离级别中使用。
...
- **快照读**：快照读又叫一致性读，读取的是历史版本的数据。不加锁的SELECT都属于快照读，即不加锁的非阻塞读。

  - 快照读的实现是基于MVCC，它在很多情况下，避免了加锁操作，降低了开销，提高了数据库的并发性能。
  - 读已提交：每次select都生成一个快照读，总是读取已提交的最新数据行。
    可重复读：第一次select生成快照读，读取上一次的快照。
    串行化：快照读会退化到当前读。
...
2. 同时解决了脏读、不可重复读等事务隔离问题（部分解决幻读），但不能解决更新丢失的问题（写写冲突解决不了）。
...
- 第一个例子：对于快照读， MVCC 并不能完全避免幻读现象。因为当事务 A 更新了一条事务 B 插入的记录，那么事务 A 前后两次查询的记录条目就不一样了，所以就发生幻读。因为update语句是当前读（读最新的commit状态的数据），而不是快照读。

  ![img](https://img-blog.csdnimg.cn/img_convert/88a1a89bca323b0595946076f559b617.webp?x-oss-process=image/format,png)
...
- 第二个例子：对于当前读，如果事务开启后，并没有执行当前读，而是先快照读，然后这期间如果其他事务插入了一条记录，那么事务后续使用当前读进行查询的时候，就会发现两次查询的记录条目就不一样了，所以就发生幻读。
  ...
  - T3 时刻：事务 A 再执行「当前读语句」 select * from t_test where id > 100 ==for updat==e; 就会得到 4 条记录，此时也发生了幻读现象。
