...
4. **持久性（Durability）**：事务结果一旦提交，就得保存起来，即使数据库发生故障也不应该对其有任何影响
...
- 未提交读（read uncommitted）：不能解决任何并发问题

- 已提交读（read committed）：只会读到其他事务的已经提交数据。只能解决脏读问题

- 可重复读（repeatable read）：一个事务中，反复读取某行数据都保持前后一致，不会被其他事务修改。只能解决脏读和不可重复读问题

- 串行化（序列化 serializable）：可转化为串行事务的执行。都能解决
