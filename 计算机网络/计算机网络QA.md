...
SYN Flood发生在TCP握手的第一次，也就是客户端向服务器发送SYN包的时候。在SYN Flood攻击中，攻击者发送大量的伪造源IP地址的SYN包，报文将永远发送不到目的地，得不到不回应，从而导致服务器的半开资源浪费直到超时释放。

防护措施：可以使用 SYN Cookie，即通过将源目地址及 IP 地址和端口号哈希为序列号，将返回的 ACK 减1后和得到原来的序列号判断是否正确，直到连接建立才分配资源。
...
慢启动：不要一开始就发送大量的数据，先探测一下网络的拥塞程度，也就是说由小到大逐渐增加拥塞窗口的大小。在没有出现丢包时每收到一个 ACK 就将拥塞窗口大小加一（单位是MSS，最大单个报文段长度），每轮次发送窗口增加一倍，呈指数增长，若出现丢包，则将拥塞窗口减半，进入拥塞避免阶段。

拥塞避免：当拥塞窗口达到慢启动阈值或出现丢包时，窗口大小不再呈指数上升，窗口每轮次加一，呈线性增长，避免增长过快导致网络拥塞。
...
快重传：当接收方发现丢了一个中间包的时候，发送三次前一个包的 ACK，于是发送端就会快速地重传，不必等待超时再重传。
